...
/**
 * Terminal component
 * Main trading dashboard with market selection, wallet, charts, order book,
 * trades feed, and admin panel toggle.
 */
export default function Terminal({ onLogout }) {
  ...
  const halted =
    marketStates.find((m) => m.symbol === symbol)?.status === "HALTED";
  ...
  return (
    <div className="min-h-full">
          ...
          <aside className="space-y-4">
            ...
            <div className="panel p-4 space-y-2">
              ...
              <p className="font-mono text-xs text-muted">
                Book frames arrive on{" "}
                <span className="text-ink">/topic/orderbook/{symbol}</span> and
                fall back to REST polling when the socket drops.
              </p>
      ...
      {/* AI Chat */}
      {tab !== "admin" && <AiChat symbol={symbol} />}
    </div>
  );
}
