...
@app.get("/health")
def health():
    """Service liveness and Ollama reachability."""
    return jsonify({
        "status": "UP",
        "service": "openex-analytics",
...
def summary():
    """Return summary of current ticks for all symbols."""
    return jsonify(simulator.summary())


@app.post("/api/chat")
def chat():
    """LangChain + Ollama agent with tool calling."""
    ...
    if not message:
        return jsonify({
            "error": "empty_message",
            "message": "Send a non-empty 'message'."
        }), 400
    ...
    try:
        ...
        return (
            jsonify({
                ...
                "message": (
                    f"The local model could not be reached ({exc}). "
