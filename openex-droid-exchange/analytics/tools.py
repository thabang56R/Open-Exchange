...
def fetch_wallet_balances(token: str | None) -> dict:
    ...
    if not token:
        return {
            ...
            "message": "No session token was supplied."
        }
    try:
        res = requests.get(
    ...
    if res.status_code == 401:
        return {
            "error": "unauthorized",
            "message": "Session expired, ask the user to sign in again."
        }
    if not res.ok:
        return {
            "error": "engine_error",
...
def fetch_open_orders(token: str | None) -> list | dict:
    """HTTP GET the Kotlin engine's orders endpoint with the caller's JWT."""
    if not token:
    ...
    try:
        ...
        if res.ok:
            return res.json()
        return {"error": "engine_error", "status": res.status_code}
    except requests.RequestException as exc:
        return {"error": "engine_unreachable", "message": str(exc)}
...
def get_wallet_balance_tool() -> str:
    """
    Return the signed-in user's wallet balances per asset from the exchange
    core engine. Use this whenever the user asks about their balance, funds,
    holdings or buying power.
    """
...
def get_open_orders_tool() -> str:
    ...
    Use this when the user asks what orders they have working or recently
    placed.
    """
...
def get_market_snapshot_tool(symbol: str = "") -> str:
    ...
    change percent, volatility and trend. Pass a symbol such as BTC-USD, or
    leave it empty for every tracked market.
    """
